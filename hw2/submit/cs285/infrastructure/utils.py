...
@ray.remote
def sample_trajectory_parrallel(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    # TODO: got this from hw1
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        n_iters = int((min_timesteps_per_batch//max_path_length) * 1.1)
        new_paths = sample_n_trajectories(env, policy, n_iters, max_path_length, render, render_mode)

        values = ray.get([get_pathlength_para.remote(path) for path in new_paths])
        while len(values) > 1:
            values = values[2:] + [add.remote(values[0], values[1])]
        result = ray.get(values[0])
        timesteps_this_batch += result
        paths += new_paths
...
def sample_n_trajectories(env, policy, ntraj, max_path_length, render=False, render_mode=('rgb_array')):
    # TODO: got this from hw1
    policy = ray.put(policy)
    env = ray.put(env)
    paths = ray.get([sample_trajectory_parrallel.remote(env, policy, max_path_length, render, render_mode) for i in range(ntraj)])
    return paths
