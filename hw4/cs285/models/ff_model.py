...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = ptu.from_numpy(normalize(obs_unnormalized, obs_mean, obs_std))# TODO(Q1) DONE
        acs_normalized = ptu.from_numpy(normalize(acs_unnormalized, acs_mean, acs_std))# TODO(Q1) DONE
        ...
        # Hint: as described in the PDF, the output of the network is thed
        # *normalized change* in state, i.e. normalized(s_t+1 - s_t).
        delta_pred_normalized = self.delta_network(concatenated_input) # TODO(Q1) DONE
        next_obs_pred = obs_unnormalized + unnormalize(ptu.to_numpy(delta_pred_normalized), delta_mean, delta_std) # TODO(Q1) DONE
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = self.forward(obs, acs, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)[0]# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        norm_delta_pred = self.forward(observations, actions, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)[1]

        loss = self.loss(norm_delta_pred, target) # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
